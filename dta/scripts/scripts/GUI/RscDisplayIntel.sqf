...
switch _mode do {
	...
	case "onUnload": {
		...
		if(_exitCode == 1) then
		{
			//debuglog format["MUFTEST - _sliderPos: %1, Display: ", _sliderPos, _display];
			
			_overcastToolBox = ((findDisplay 26) displayctrl 2602); //overcast ToolBox in RscDisplayArcadeMap
			
			//TODO - find out what command to use to set selected item in ToolBox
			...
			((findDisplay 26) displayctrl 2601) sliderSetPosition ( (10.0 / 23.9167) * (_hours + (_minutes / 60)) );  //set time slider position in RscDisplayArcadeMap
