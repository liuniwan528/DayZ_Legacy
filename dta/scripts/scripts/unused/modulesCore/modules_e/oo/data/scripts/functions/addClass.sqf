scriptName "DZ\ModulesCore\modules_e\oo\data\scripts\functions\addClass.sqf";
/*
	File: addClass.sqf
...
if (isNil "_this") exitWith {debugLog "Log: [addClass] no parameters defined!"; false};
if ((typeName _this) != (typeName "")) exitWith {debugLog "Log: [addClass] class must be a String!"; false};
...
if ((configName _class) == "") exitWith {debugLog "Log: [addClass] class " + _this + " does not exist in CfgOO!"; false};
...
if ((configName _methods) != "") then 
{
	...
	for "_i" from 0 to ((count _methods) - 1) do 
	{
		...
		if ((typeName _methodName) != (typeName "")) exitWith {debugLog "Log: [addClass] method name should be a String!"; _return = false};
		if (_methodName == "") exitWith {debugLog "Log: [addClass] method name cannot be empty!"; _return = false};
		
		_methodVar = "BIS_OO_" + _className + "_" + _methodName;
		
		_methodScript = getText (_method >> "script");
		if ((typeName _methodScript) != (typeName "")) exitWith {debugLog "Log: [addClass] method script should be a String!"; _return = false};
		if (_methodScript == "") exitWith {debugLog "Log: [addClass] method script cannot be empty!"; _return = false};
...
{
	BIS_OO_createObject = compile (preprocessFileLineNumbers "DZ\ModulesCore\modules_e\oo\data\scripts\functions\createObject.sqf");
	BIS_OO_validateParameters = compile (preprocessFileLineNumbers "DZ\ModulesCore\modules_e\oo\data\scripts\functions\validateParameters.sqf");
};

debugLog ("Log: [addClass] loaded class: " + _className);
_return