...
private ["_parentDisplayDefault","_parentClass","_parentDisplay","_tags","_deepnessLimit","_onClick","_mapSize","_scale","_display","_fnc_validClass","_fnc_countSubClasses","_deepnessMax","_n","_structure","_map","_parentParams","_subClassesCount","_info","_footer","_fade","_selected"];
...
_parentClass = [_this,0,configfile >> "CfgORBATDefault",[configfile]] call (uinamespace getvariable "bis_fnc_param");
...
_scale = 64 / _mapSize;
with uinamespace do {
	RscDisplayORBAT_scaleMin = _scale;
	RscDisplayORBAT_scaleMax = _scale * 4;
...
_fnc_countSubclasses = {
	private ["_class","_classDeepness","_subclass","_count"];
		...
		_parents = _class call bis_fnc_returnparents;
		{
			for "_c" from 0 to (count _x - 1) do {
				...
				if (isclass _subclass) then {
					_count = _count + ([_subclass,_classDeepness + 1] call _fnc_countSubclasses);
				};
			};
		} foreach _parents;
...
BIS_fnc_ORBAT_loadConfig = {
	private ["_class","_classParams","_deepness","_onClickID","_subclass","_result","_text","_texture","_textureMarker","_textureName","_size","_color","_textureSize","_classParams","_countChild"];
	_class = _this select 0;
	...
	if (_onClickID >= 0) then {
		...
		_localOverlay set [count _localOverlay,[["\DZ\ui\data\GUI\Rsc\RscDisplayORBAT\shortcut_ca.paa",[0,0,0,1],1,1,0,"",false]]];
	};
	...
	if (_class call _fnc_validClass && _deepness < _deepnessLimit) then {
		...
		_parents = _class call bis_fnc_returnparents;
		{
			for "_c" from 0 to (count _x - 1) do {
				...
				if (isclass _subclass) then {
					//--- We must go deeper!
					_result = _result + [[_subclass,_deepness + 1] call BIS_fnc_ORBAT_loadConfig];
...
BIS_fnc_ORBAT_loadArray = {
	...
	if (
		([_posX,_posY] distance (_map ctrlmapscreentoworld BIS_fnc_ORBAT_mousePos)) < (BIS_fnc_ORBAT_step / 5)
		&&
		_colorAlpha > 0
	) then {
		[_params,_display,BIS_fnc_ORBAT_mousePos] call bis_fnc_ORBATTooltip;
	...
	_map drawicon [
		...
		[0,0,0,1],
		[_posX - _textOffsetX,_posY - _textOffsetYLocal],
		//[_posX,_posY],
		_iconSize * 1.1,
		0,
		0,
...
BIS_fnc_ORBAT_draw = {
	...
	_stepX = (BIS_fnc_ORBAT_step / 1.5) * safezoneH;
	_stepY = -BIS_fnc_ORBAT_step * safezoneH;
	_stepYhalf = _stepY / 2;

	_textOffsetX = 4.2 * safezoneH;
...
BIS_fnc_ORBAT_posStart = [	
	...
	1000 + (_deepnessMax * -BIS_fnc_ORBAT_step) / 2,
	0
];
_map ctrlmapanimadd [0,((0.0007 * _subclassesCount)) / BIS_fnc_ORBAT_sizeCoef,BIS_fnc_ORBAT_posStart];
...
BIS_fnc_ORBAT_camera = if (count allmissionobjects "Camera" == 0) then {
