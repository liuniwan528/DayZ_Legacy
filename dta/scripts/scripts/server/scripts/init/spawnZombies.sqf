...
fnc_spawnZombieTypes = {
	private ["_typenames","_types","_nmt"];
	_typenames = _this;
	_types = [];
	{ _nmt = format["typesZ%1",_x]; _types = _types + getArray(configFile >> "CfgSpawns" >> _nmt) } forEach _typenames;
	_types
};

//DZ_ZombieTypes = getArray(_cfgSpawns >> "types");
DZ_ZombieTypes = ["Female","Male","Hermit","Soldier","Crouch"] call fnc_spawnZombieTypes;
...
//spawn zombies in spawns areas
{
	...
	_types = getArray(_class >> "typesZ") call fnc_spawnZombieTypes;
...
//057		diag_log format ["Zombie Spawning >>> %1 :: %2",_x,(count _agentTypes) ];
	};
} forEach _spawnsArray;
