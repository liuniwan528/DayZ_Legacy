...
switch _state do
{
	...
	case 1: 
	{
		...
		//E2
		if (_item isKindOf "Medical_BandageDressing") exitWith
		{
		...
		{
			_kindling_items set [2, [_item, _book_energy]]; //[item, energy value]
		};
		
		//E4
		if (_item isKindOf "Consumable_Paper") exitWith
		{
			_kindling_items set [3, [_item, _paper_energy]]; //[item, energy value]
		};
	};
