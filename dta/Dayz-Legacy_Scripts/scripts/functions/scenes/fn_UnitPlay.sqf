//scriptName "modules_e\Functions\scenes\fn_UnitPlay.sqf";
/*
	File: fn_UnitPlay.sqf
	...
	a)	_capturedData = [[0,[8208.26,1953.13,296.04],[0,1,0],[0.000174453,0,1],[0,0,0]]];
	...
	b)	_capturedData = [[0,[8208.26,1953.13,296.04],[0,1,0],[0.000174453,0,1],[0,0,0]]];
	...
	c)	_capturedData = [[0,[8208.26,1953.13,296.04],[0,1,0],[0.000174453,0,1],[0,0,0]]];
...
while { ((_selectedData <= _totalData) && !(_endSeq)) } do
{
	...
	_tmpVecDir = _currentData select 2;
	_tmpVecUp = _currentData select 3;
	_tmpVel = _currentData select 4;
	
	_tmpPosNext = _currentDataNext select 1;
	_tmpVecDirNext = _currentDataNext select 2;
	_tmpVecUpNext = _currentDataNext select 3;
	_tmpVelNext = _currentDataNext select 4;
	
	while { ((_tmpTimeNext >= _currentTime) && !(_endSeq)) } do
	{
			_tmpTimeDiff = (_currentTime - _tmpTime) / (_tmpTimeNext - _tmpTime);
			_targetUnit setVelocityTransformation [_tmpPos, _tmpPosNext, _tmpVel, _tmpVelNext, _tmpVecDir, _tmpVecDirNext, _tmpVecUp, _tmpVecUpNext, _tmpTimeDiff];
	...
	if (_tmpTime == 0) then
	{
		...
		_targetUnit setVelocityTransformation [_tmpPos, _tmpPos, _tmpVel, _tmpVel, _tmpVecDir, _tmpVecDir, _tmpVecUp, _tmpVecUp, 0];
