...
//	  textLogFormat ["INV_ADD _x slot: %1 ", _x, _success];
	...
	if (_success) then
	{
	  ...
	  {
	    if (_item == (configfile >> "cfgweapons" >> (configName _item))) then {_unit createInInventory configName _item;};
	    if (_item == (configfile >> "cfgmagazines" >> (configName _item))) then {_unit createInInventory configName _item;};
	  }
	  else
	  {
	    if ((typeName _item) == "STRING") then
	    {
	      if (isClass (configfile >> "cfgweapons" >> (_item))) then {_unit createInInventory _item;}
	      else
	      {
	        if (isClass (configfile >> "cfgmagazines" >> (_item))) then {_unit createInInventory _item;};
	      };
	    };
	  };
	};
