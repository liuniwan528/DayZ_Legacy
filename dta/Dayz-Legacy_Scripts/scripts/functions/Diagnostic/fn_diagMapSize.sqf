	...
	if (isclass _class) then {
		...
		if ({(configname _class) iskindof _x} count _exclude == 0) then {
			...
			if (_scope > 0) then {
				...
				_classSizes set [count _classSizes,_size];

				deletevehicle _obj;
			} else {
				//_sizeNames set [count _sizeNames,configname _class];
				_classSizes set [count _classSizes,0];
			};
		};
	};
	...
	switch (_mode) do {
		...
		case "config": {
			if (typename _sizeLink == typename 0) then {
				...
				_result = _result + _br +
					"#define " + _macroName + _class + "	mapSize = " + str _size + ";";
			};
		};
		case "config_current": {
			_sizeText = if (typename _sizeLink == typename 0) then {
			...
			_result = _result + _br +
			"#define " + _macroName + _class + _sizeText;
		};
	};
