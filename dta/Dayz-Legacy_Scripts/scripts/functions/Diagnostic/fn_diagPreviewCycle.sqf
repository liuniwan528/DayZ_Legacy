/*
	...
	_this select 2 (Optional): DISPLAY - parent display
...
_pos = [_this,2,[position player,10,0] call bis_fnc_relpos] call bis_fnc_param;
_parentDisplay = [_this,1,displaynull,[displaynull]] call bis_fnc_param;
	...
	case "cfgvehicles": {
		...
		_ctrlList ctrladdeventhandler [
			...
			"
				...
				_ctrllabel = _display displayctrl 1000;
				_ctrllabel ctrlsettext _vehicle;
			"
		];
	...
	case "cfgweapons": {
		_ctrlList ctrladdeventhandler [
			...
			"
				...
				_weaponName = [configfile >> 'CFgWeapons' >> _weapon] call bis_fnc_displayname;
				_pos = bis_fnc_diagPreviewCycle_pos;
				deletevehicle (missionnamespace getvariable ['bis_fnc_diagPreviewCycle_object',objnull]);
				_object = createvehicle ['weaponholder',_pos,[],0,'none'];
				_object setpos _pos;
				_object setdir 0;
