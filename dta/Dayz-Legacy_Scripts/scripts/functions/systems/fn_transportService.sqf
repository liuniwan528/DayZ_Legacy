...
if (isnil "RE") then {[] execVM "\DZ\ModulesCore\Modules\MP\data\scripts\MPframework.sqf"};
...
BIS_transportServiceFSM = [_caller, _veh, _passedPos, _allowed, _plannedLZ, _supportType] execFSM "DZ\ModulesCore\modules\functions\systems\fn_transportService.fsm";
...
[_caller, _veh, _allowed, _plannedLZ, _supportType, _secopScope, _passedPos] spawn {
	...
	while {_caller in (_veh getVariable "supPool")} do {
		if (_lastMarkedPos distance (_secopScope getVariable "coordsUpdate") > 0) then {
			...
			BIS_transportServiceFSM = [_caller, _veh, _lastMarkedPos, _allowed, _plannedLZ, _supportType] execFSM "DZ\ModulesCore\modules\functions\systems\fn_transportService.fsm"
		};
		sleep 1
	}
};