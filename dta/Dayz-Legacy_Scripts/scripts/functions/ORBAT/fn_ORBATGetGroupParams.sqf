...
if (isclass _cfgSize) then {
	...
	_sizeGenders = getarray (_cfgSize >> "gender");
	_sizeGenderID = _sizeGenders find language;
	if (_sizeGenderID >= 0) then {_sizeGender = [_sizeGenders,_sizeGenderID + 1,0,[0]] call bis_fnc_paramIn;};
} else {
	if (_size != "") then {
...
if (isclass _cfgType) then {
	...
	_cfgTypeName = _cfgType >> "name";
	if (isarray _cfgTypeName) then {
		_typeName = getarray _cfgTypeName;
		_typeName resize 3;
		_typeName = _typeName select (_sizeGender);
		if (isnil "_typeName") then {_typeName = "";};
	} else {
		gettext _cfgTypeName
	};
} else {
	if (_type != "") then {
...
_color = if (istext _cfgColor) then {
	...
	if (isclass _colorMarker) then {getarray (_colorMarker >> "color")} else {_sideColor};
} else {
	getarray _cfgColor;
};
...
if (isclass _cfgCommander) then {
	...
	_commanderID = 42;
	{
		_commanderID = _commanderID + _x;
	} foreach _classArray;
	_commanderID = _commanderID % (count _cfgCommander);
...
_assetsText = "";
{
	...
	_assetIcon = gettext (configfile >> "CfgVehicles" >> _asset >> "icon");
