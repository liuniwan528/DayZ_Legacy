...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if (count _input == 1) exitwith {
		...
		if !(isnil "_params") then {
			...
			if (count _actions > 0) then {
				_heli removeaction (_actions select 0);
				_heli removeaction (_actions select 1);
				(_actions select 1) call bis_fnc_onLoad;
			};
	...
	if (!isnil "_currentTarget" && !_replace) exitwith {
		["'%1' already taken.",_rendertarget] call bis_fnc_log;
	};
	...
	_actions = if !(isnull _heli) then {
		_action_ON = _heli addAction [
			localize "STR_HSIM_US_CORE_P1_03_PIP_ON",
			"BIS_fnc_PIP" call bis_fnc_functionPath,
			[1,_rendertarget],
			0.5,
			true,
			true,
			"",
			format ["isnull (uinamespace getvariable ['%1',displaynull]) && isPipEnabled",DISPLAY_VAR],
			"action_screen1",
			3,
			0.08,
			1 + 2 + 4 + 16 + 64,
			1 + 2,
			"<img image='HSim\UI_H\data\igui\ico_insp_look_ca' size='2' shadow='true' />",
			"<br /><t size='0.8'>" + (localize "STR_HSIM_US_CORE_P1_03_PIP_ON_SHORT") + "</t>"
		];
		_action_OFF = _heli addAction [
			localize "STR_HSIM_US_CORE_P1_03_PIP_OFF",
			"BIS_fnc_PIP" call bis_fnc_functionPath,
			[0],
			0.5,
			true,
			true,
			"",
			format ["!isnull (uinamespace getvariable ['%1',displaynull])",DISPLAY_VAR],
			"action_screen1",
			3,
			0.08,
			1 + 2 + 4 + 16 + 64,
			1 + 2,
			"<img image='HSim\UI_H\data\igui\ico_OFF_ca' size='2' shadow='true' />",
			"<br /><t size='0.8'>" + (localize "STR_HSIM_US_CORE_P1_03_PIP_OFF_SHORT") + "</t>"
		];
		...
		_index = [
			{
				...
				if (_IGUI) then {
					[_heli,_heli,-1,[1,_this]] call bis_fnc_pip;
				};
			},
			_rendertarget
		] call bis_fnc_onLoad;
