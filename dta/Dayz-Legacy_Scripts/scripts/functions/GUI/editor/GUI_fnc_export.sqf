...
_data = if (!_shift && count GUI_listElements > 0) then {[GUI_version,GUI_grid]} else {[]};
	...
	if ((_idcPositionType <= 2 || !isnil "_childIdc") && (!(_idcClass in _listClasses) || _isComment)) then {
		...
		if (_shift) then {
			...
			if !(_isComment) then {
				...
				if (_symbol_ddot in _idcClassArray) then {
									...
									if ({_textParam in _listBanned} count _listParam == 0) then {
										...
										if (typename _x == typename "") then {
											...
											if (_forEachIndex in [GUI_param_text,GUI_param_tooltip]) then {
												if !((_idc call GUI_fnc_elementLayer) in [GUI_layer_pictureshadow,GUI_layer_picturenoshadow]) then {
													_xArray = toarray _x;
