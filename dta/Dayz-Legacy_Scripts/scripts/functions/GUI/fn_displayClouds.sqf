...
/*
					...
					if (!isnull player) then {
						_idc = _idcBase - 2;
						CONTROL ctrlsettext "DZ\ui\data\ui_gradient_top_ca.paa";
					...
					if (_jokeCartoon) then {
						_idc = _idcBase - 1;
						CONTROL ctrlsettext "DZ\ui\data\editor_jasno.paa";
						_sizeH = safezoneH / 5;
...
/*
					...
					for "_i" from 0 to (_countClouds - 1) do {
						...
						//--- Create
						_path = '\DZ\data\data\data\mrak_0%1_ca.paa';
						_size =  _i + 1;
						_sizeH = (safezoneH) * sqrt(_i1);
						if (_jokeCartoon) then {
							_sizeH = _sizeH / 3;
							_path = "DZ\ui\data\editor_zatazeno.paa";
						};
						_sizeW = _sizeH * 3/4;
						_pos = [
