//scriptName "Functions\misc\fn_taskHandler.sqf";
/*******************************************************************************
 *
 *	version: 		1.0
 *	name:			Task handler
...
if (isServer && !(local (_this select 0))) then {
	[(_this select 0), (_this select 1)] spawn {
		...
		if ((_this select 0) call BIS_checkAITasks) then {(_this select 0) setVariable ["BIS_tasksPriorityLadder", (_this select 1), TRUE]; BIS_AITasks = (_this select 0) execFSM "DZ\ModulesCore\modules\functions\misc\fn_AITasks.fsm"}
	}
};
...
if ((_this select 0) call BIS_checkAITasks) then {
	(_this select 0) setVariable ["BIS_tasksPriorityLadder", (_this select 1), TRUE]; BIS_AITasks = (_this select 0) execFSM "DZ\ModulesCore\modules\functions\misc\fn_AITasks.fsm"
} else {
	_this spawn {
...
if (isPlayer (_this select 0) || {isPlayer _x} count units (_this select 0) > 0) then {
	BIS_taskHandler = _this execFSM "DZ\ModulesCore\modules\functions\misc\fn_taskHandler.fsm";
