...
//--- Terminate when some other savegame is already waiting
if (!isnil "bis_fnc_savegame_wait") exitwith {};
...
_condition = {
	{
		...
		_alt = (position _veh select 2) - 20;
		_speed = speed _veh;
		_maxSpeed = getnumber (configfile >> "cfgvehicles" >> (typeof _veh) >> "maxspeed");
		if (_maxspeed == 0) then {_maxspeed = _speed + 1};
		...
		||
		//--- Too low
		(-_alt > (velocity _veh select 2)*2 && _alt < 50)
	} count _units == 0
};

//--- Save when clear
bis_fnc_savegame_wait = [_units,_condition] spawn {
