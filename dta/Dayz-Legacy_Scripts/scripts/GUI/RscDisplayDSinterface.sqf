//--- Script uses different structure because previous script was FUBAR in given time
...
switch _action do {
	...
	case"vote_kick": {
		serverCommand ("#vote kick " + _selectedName);
	};
	case"vote_admin": {
		serverCommand ("#vote admin " + _selectedName);
	};
	case"login": {
		serverCommand ("#login " + _password);
	};
...
if ((_action == "login") or (_action == "logout") or (_action == "kick") or (_action == "vote_admin")) then	{
	...
	_test = ["reload",_data] call (uiNamespace getVariable 'RscDisplayDSinterface_script');
};
...
switch _mode do
{
	...
	{
		//--- set player's name
		(_display displayctrl 109) ctrlSetText ([] call (uinamespace getvariable 'BIS_fnc_playerName'));
		
		//toUpper Title
		_control = _display displayctrl 1000;
		_control ctrlSetText (toUpper (ctrlText _control));
		
		//Count missions and players - works but can be done better (wait until ListBox is filled, than count), or use event handler for changing count of ListBox
		_display displayaddeventhandler
		[
			"mousemoving",
			"
				((_this select 0) displayctrl 1002) ctrlsettext format[""Missions: %1"", (lbsize ((_this select 0) displayctrl 102))];
				((_this select 0) displayctrl 1003) ctrlsettext format[""Players: %1"", (lbsize ((_this select 0) displayctrl 101))];
			"
		];
	};
};
