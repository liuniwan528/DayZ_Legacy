...
for "_i" from 0 to ((count _path) - 1) do 
{
	...
	if ((typeName _array) != (typeName [])) exitWith {_return = false; debugLog "Log: [setNestedElement] One of the elements in the given path is not an Array!"};
	if (_nextIndex > ((count _array) - 1)) exitWith {_return = false; debugLog "Log: [setNestedElement] One of the elements in the given path is outside the scope of its parent Array!"};
