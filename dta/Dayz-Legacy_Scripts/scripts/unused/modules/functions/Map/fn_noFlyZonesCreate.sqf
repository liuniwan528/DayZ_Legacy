...
if (isnil "DZnoFlyZonesMarkes") then {DZnoFlyZonesMarkes = [];};
{deletemarker _x} foreach DZnoFlyZonesMarkes;
DZnoFlyZonesMarkes = [];

//--- Delete previous zones
if (isnil "DZnoFlyZones") then {DZnoFlyZones = [];};
{deletevehicle _x} foreach DZnoFlyZones;
DZnoFlyZones = [];
...
for "_i" from 0 to (count _cfgNoFlyZones - 1) do {
	...
	missionnamespace setvariable [format ["DZnofly_%1",_class],_trigger];
	DZnoFlyZones = DZnoFlyZones + [_trigger];
};
	...
	if (_triggerArea select 3) then {
		...
		for "_i" from 0 to 3 do {
			...
			for "_d" from 0 to (_dis - _lineDistanceLocal) step _lineDistanceLocal do {
				...
				if ({[_x,_pos] call bis_fnc_intrigger} count (DZnoFlyZones - [_x]) == 0) then {
					...
					DZnoFlyZonesMarkes set [count DZnoFlyZonesMarkes,_marker];
